...
def inference(start_time, frame, main_boxes, empty_boxes, main_model, empty_model):
    result = []
    main_crops = crop_image(frame, main_boxes, (224, 224))
    empty_crops = crop_image(frame, empty_boxes, (224, 224))

    for i in range(len(main_crops)):
        main_crops[i] = tf.keras.applications.efficientnet.preprocess_input(np.expand_dims(main_crops[i], axis=0))
        empty_crops[i] = tf.keras.applications.efficientnet.preprocess_input(np.expand_dims(empty_crops[i], axis=0))

    empty_pred = empty_model.predict(empty_crops)
    main_pred = main_model.predict(main_crops)
...
if __name__ == "__main__":
    ...
    for left_frame, right_frame in zip(left_test_images, right_test_images):
        ...
        left_result, left_end_time = inference(start_time, left_frame, lm_boxes, lem_boxes, main_model, empty_model)
        right_result, right_end_time = inference(start_time, right_frame, rm_boxes, rem_boxes, main_model, empty_model)
