...
public struct NetworkResult
{
    public NetworkResult(int numboxes, int numclasses, int numscores)
    {
        PredictionBoxes = numboxes;
        PredictionClasses = numclasses;
        PredictionScores = numscores;
    }

    public int PredictionBoxes { get; }
    public int PredictionClasses { get; }
    public int PredictionScores { get; }
}
...
public class NetworkModel
{
        ...
        VideoFrame inputFrame)
    {
        ...
        if (outboxes.Count() > 0)
        {
            return new NetworkResult(outboxes.Count(), outclasses.Count(), outscore.Count());
        }

        else:
        {
            return new NetworkResult(0, 0, 0); ;
        }
    }
#endif
}