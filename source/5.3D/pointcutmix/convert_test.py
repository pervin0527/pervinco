...
state_dict = torch.load('checkpoints/modelnet40_dgcnn_kcutmix_train/best.pth')
...
print(loss.item() / (j + 1), 100. * correct.item() / total)
    ...
    # torch.onnx.export(model, points, './onnx/result.onnx')
# 
# onnx_model = onnx.load('onnx/result.onnx')
