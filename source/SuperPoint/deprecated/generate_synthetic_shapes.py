...
def draw_interest_points(img, points):
    ...
    for i in range(points.shape[0]):
        cv2.circle(img_rgb, (int(points[i][0]), int(points[i][1])), 5, (0, 255, 0), -1)
    return img_rgb
...
def draw_polygon(img, max_sides=8):
    ...
    x = np.random.randint(rad, img.shape[1] - rad)
    ...
    points = np.array([[int(x + max(np.random.rand(), 0.4) * rad * math.cos(a)), int(y + max(np.random.rand(), 0.4) * rad * math.sin(a))] for a in angles])
    ...
    corner_angles = [angle_between_vectors(points[(i-1) % num_corners, :] - points[i, :], points[(i+1) % num_corners, :] - points[i, :]) for i in range(num_corners)]
...
def draw_multiple_polygons(img, max_sides=8, nb_polygons=30, **extra):
    ...
    for i in range(nb_polygons):
        ...
        slices = np.linspace(0, 2 * math.pi, num_corners + 1)
        angles = [slices[i] + np.random.rand() * (slices[i+1] - slices[i]) for i in range(num_corners)]
        new_points = [[int(x + max(np.random.rand(), 0.4) * rad * math.cos(a)), int(y + max(np.random.rand(), 0.4) * rad * math.sin(a))] for a in angles]
        ...
        corner_angles = [angle_between_vectors(new_points[(i-1) % num_corners, :] - new_points[i, :], new_points[(i+1) % num_corners, :] - new_points[i, :]) for i in range(num_corners)]
        mask = np.array(corner_angles) < (2 * math.pi / 3)
        new_points = new_points[mask, :]
        num_corners = new_points.shape[0]
        ...
        if intersect(segments[:, 0:2, None], segments[:, 2:4, None], new_segments[:, 0:2, :], new_segments[:, 2:4, :], 3) or overlap(np.array([x, y]), rad, centers, rads):
            continue
...
def draw_star(img, nb_branches=6):
    ...
    points = np.array([[int(x + max(np.random.rand(), 0.3) * rad * math.cos(a)), int(y + max(np.random.rand(), 0.3) * rad * math.sin(a))] for a in angles])
    points = np.concatenate(([[x, y]], points), axis=0)
    background_color = int(np.mean(img))

    for i in range(1, num_branches + 1):
        col = get_random_color(background_color)
        cv2.line(img, (points[0][0], points[0][1]), (points[i][0], points[i][1]), col, thickness)
...
def draw_checkerboard(img, max_rows=7, max_cols=7, transform_params=(0.05, 0.15)):
    ...
    rows = np.random.randint(3, max_rows)
    cols = np.random.randint(3, max_cols)
    ...
    alpha_affine = np.max(img.shape) * (transform_params[0] + np.random.rand() * transform_params[1])
    center_square = np.float32(img.shape) // 2
    min_dim = min(img.shape)
    square_size = min_dim // 3
    pts1 = np.float32([center_square + square_size, [center_square[0]+square_size, center_square[1]-square_size], center_square - square_size, [center_square[0]-square_size, center_square[1]+square_size]])
    ...
    warped_col0 = np.add(np.sum(np.multiply(warped_points, perspective_transform[0, :2]), axis=1), perspective_transform[0, 2])
    warped_col1 = np.add(np.sum(np.multiply(warped_points, perspective_transform[1, :2]), axis=1), perspective_transform[1, 2])
    warped_col2 = np.add(np.sum(np.multiply(warped_points, perspective_transform[2, :2]), axis=1), perspective_transform[2, 2])
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            colors[i * cols + j] = col
            cv2.fillConvexPoly(img, np.array([(warped_points[i * (cols + 1) + j, 0], warped_points[i * (cols + 1) + j, 1]), (warped_points[i * (cols + 1) + j + 1, 0], warped_points[i * (cols + 1) + j + 1, 1]),
                                             (warped_points[(i + 1) * (cols + 1) + j + 1, 0], warped_points[(i + 1) * (cols + 1) + j + 1, 1]),
                                             (warped_points[(i + 1) * (cols + 1) + j, 0], warped_points[(i + 1) * (cols + 1) + j, 1])]), col)
...
def draw_stripes(img, max_nb_cols=13, min_width_ratio=0.04, transform_params=(0.05, 0.15)):
    ...
    board_size = (int(img.shape[0] * (1 + np.random.rand())), int(img.shape[1] * (1 + np.random.rand())))
    col = np.random.randint(5, max_nb_cols)
    ...
    alpha_affine = np.max(img.shape) * (transform_params[0]
                                        + np.random.rand() * transform_params[1])
    center_square = np.float32(img.shape) // 2
    ...
    for i in range(col):
        ...
        cv2.fillConvexPoly(img, np.array([(warped_points[i, 0], warped_points[i, 1]),
...
def draw_cube(img, min_size_ratio=0.2, min_angle_rot=math.pi / 10, scale_interval=(0.4, 0.6), trans_interval=(0.5, 0.2)):
    background_color = int(np.mean(img))
    ...
    rotation_1 = np.array([[math.cos(rot_angles[0]), -math.sin(rot_angles[0]), 0], [math.sin(rot_angles[0]), math.cos(rot_angles[0]), 0], [0, 0, 1]])
    rotation_2 = np.array([[1, 0, 0], [0, math.cos(rot_angles[1]), -math.sin(rot_angles[1])], [0, math.sin(rot_angles[1]), math.cos(rot_angles[1])]])
    rotation_3 = np.array([[math.cos(rot_angles[2]), 0, -math.sin(rot_angles[2])], [0, 1, 0], [math.sin(rot_angles[2]), 0, math.cos(rot_angles[2])]])
    scaling = np.array([[scale_interval[0] + np.random.rand() * scale_interval[1], 0, 0], [0, scale_interval[0] + np.random.rand() * scale_interval[1], 0], [0, 0, scale_interval[0] + np.random.rand() * scale_interval[1]]])
    trans = np.array([img.shape[1] * trans_interval[0] + np.random.randint(-img.shape[1] * trans_interval[1], img.shape[1] * trans_interval[1]), img.shape[0] * trans_interval[0] +
                      np.random.randint(-img.shape[0] * trans_interval[1], img.shape[0] * trans_interval[1]), 0])
    cube = trans + np.transpose(np.dot(scaling, np.dot(rotation_1, np.dot(rotation_2, np.dot(rotation_3, np.transpose(cube))))))
    cube = cube[:, :2]
    ...
    for i in [0, 1, 2]:
        for j in [0, 1, 2, 3]:
            ...
            cv2.line(img, (cube[faces[i][j], 0], cube[faces[i][j], 1]), (cube[faces[i][(j + 1) % 4], 0], cube[faces[i][(j + 1) % 4], 1]), col_edge, thickness)
...
def generate_background(size=(960, 1280), nb_blobs=100, min_rad_ratio=0.01, max_rad_ratio=0.05, min_kernel_size=50, max_kernel_size=300):
    ...
    dim = max(size)
    cv2.randu(img, 0, 255)
    cv2.threshold(img, np.random.randint(256), 255, cv2.THRESH_BINARY, img)
    background_color = int(np.mean(img))
    blobs = np.concatenate([np.random.randint(0, size[1], size=(nb_blobs, 1)), np.random.randint(0, size[0], size=(nb_blobs, 1))], axis=1)

    for i in range(nb_blobs):
        col = get_random_color(background_color)
        cv2.circle(img, (blobs[i][0], blobs[i][1]), np.random.randint(int(dim * min_rad_ratio), int(dim * max_rad_ratio)), col, -1)
...
def get_data(img_size, resize, b):
        image = generate_background(size=img_size, min_rad_ratio=0.02, max_rad_ratio=0.031, min_kernel_size=150, max_kernel_size=500)

        option = np.random.randint(0, 8)
        if option == 0:
            points = draw_lines(image)

        elif option == 1:
            points = draw_polygon(image)

        elif option == 2:
            points = draw_multiple_polygons(image)

        elif option == 3:
            points = draw_ellipses(image)

        elif option == 4:
            points = draw_star(image, 6)

        elif option == 5:
            points = draw_checkerboard(image)

        elif option == 6:
            points = draw_stripes(image)

        elif option == 7:
            points = draw_cube(image)
        
        # points = np.array(points)
        # points = np.flip(points, 1)
        # image = cv2.GaussianBlur(image, (b, b), 0)
        # points = (points * np.array(resize, np.float64) / np.array(img_size, np.float64))
        # image = cv2.resize(image, tuple(resize[::-1]),interpolation=cv2.INTER_LINEAR)
...
if __name__ == "__main__":
    ...
    splits = {"train" : 10000, "valid" : 200, "test" : 500}
    for name, num in splits.items():
        save_dir = f"/data/Datasets/Synthetic_shapes/{name}"
        ...
        for index in tqdm(range(num)):
            ...
            sample = draw_interest_points(image, points)

            cv2.imwrite(f"{save_dir}/images/{index}.png", image)
            np.save(f"{save_dir}/points/{index}.npy", points)