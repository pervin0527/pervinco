...
def dump_primitive_data(primitive, config):
    temp_dir = Path(data_path, "synthetic_shapes" + ('_{}'.format(config['data']['suffix'])), primitive)
...
def generate_dataset():
    ...
    basepath = Path(data_path, 'synthetic_shapes' + ('_{}'.format(config['data']['suffix']) if config['data']['suffix'] is not None else ''))
    basepath.mkdir(parents=True, exist_ok=True)

    splits = {s: {'images': [], 'points': []} for s in ['training', 'validation', 'test']}
    for primitive in primitives:
        if not Path(data_path, 'synthetic_shapes' + ('_{}'.format(config['data']['suffix'])), primitive).exists():
            dump_primitive_data(primitive, config)

        # Gather filenames in all splits, optionally truncate
        truncate = config['data']['truncate'].get(primitive, 1)
        path = Path(data_path, 'synthetic_shapes' + ('_{}'.format(config['data']['suffix']) if config['data']['suffix'] is not None else ''), primitive)
        for s in splits:
