...
def get_callbacks(params, val_dataset=None):
  ...
  if params['moving_average_decay']:
    ...
    ckpt_callback = tf.keras.callbacks.ModelCheckpoint(os.path.join(params['model_dir'], 'ckpt-{epoch:d}'),
                                                       verbose=params['verbose'],
                                                      #  save_freq=params['save_freq'],
                                                       save_weights_only=True,
                                                       save_best_only=True,
    ...
    tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=os.path.join(params['model_dir']))

    if params['es']:
      early_stopper = tf.keras.callbacks.EarlyStopping(monitor=params['es_monitor'], patience=params['es_patience'], verbose=params['verbose'])
      callbacks = [ckpt_callback, early_stopper]
