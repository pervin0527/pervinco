...
def train_normalize(image, label):
    image = tf.cast(image, tf.float32)
    image = tf.numpy_function(func=train_aug, inp=[image], Tout=tf.float32)
...
def valid_normalize(image, label):
    image = tf.cast(image, tf.float32)
    image = tf.numpy_function(func=valid_aug, inp=[image], Tout=tf.float32)
    image = tf.keras.applications.resnet.preprocess_input(image)
...
def make_train_ds(images, labels):
    images = tf.data.Dataset.from_tensor_slices(images)
    labels = tf.data.Dataset.from_tensor_slices(labels)

    dataset = tf.data.Dataset.zip((images, labels))
    dataset = dataset.repeat()    
    dataset = dataset.map(train_normalize, num_parallel_calls=AUTOTUNE)
...
def make_valid_ds(images, labels):
    images = tf.data.Dataset.from_tensor_slices(images)
    labels = tf.data.Dataset.from_tensor_slices(labels)

    dataset = tf.data.Dataset.zip((images, labels))
    dataset = dataset.repeat()    
    dataset = dataset.map(valid_normalize, num_parallel_calls=AUTOTUNE)
    dataset = dataset.batch(BATCH_SIZE)
    dataset = dataset.prefetch(AUTOTUNE)
...
def train_cross_validate():
    ...
    for i in range(N_FOLD):
        ...
        model = get_model()
        model.fit(make_train_ds(train_x, train_y), 
                  steps_per_epoch = TRAIN_STEPS_PER_EPOCH,
                  epochs = EPOCHS,
                  validation_data = make_valid_ds(valid_x, valid_y),
...
if __name__ == "__main__":
    ...
    train_transforms = A.Compose([
        A.Resize(RE_SIZE, RE_SIZE, p=1),
        A.OneOf([
            A.HorizontalFlip(p=0.8),
            A.VerticalFlip(p=0.8),
            A.RandomRotate90(p=0.9),], p=1),
    ])
