...
if __name__ == "__main__":
    ...
    SAVE_DIR = "/data/Datasets/SPC/full-name1/test"
    ...
    for n in range(AUG_N):
        ...
        for i in tqdm(range(transformed_ds_len)):
            ...
            try:
                ...
                cv2.imwrite(f'{SAVE_DIR}/images/{n}_{i}.jpg', transformed_data['image'])
                height, width = transformed_data['image'].shape[:-1]
                write_xml(f"{SAVE_DIR}/annotations", transformed_data['bboxes'], transformed_data['labels'], f'{n}_{i}', height, width, 'albumentations')
        ...
        print(len(get_files(f"{SAVE_DIR}/images")), len(get_files(f"{SAVE_DIR}/annotations")))