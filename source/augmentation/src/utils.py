...
def visualize(image, boxes, labels, format="pascal_voc"):
    for bb, c in zip(boxes, labels):
        ...
        if format == "pascal_voc":
            cv2.rectangle(image, (int(bb[0]), int(bb[1])), (int(bb[2]), int(bb[3])), (255, 0, 255), thickness=5)
            cv2.putText(image, str(c), (int(bb[0]), int(bb[1])), cv2.FONT_HERSHEY_PLAIN, 1, (255, 0, 255))            

        elif format == "albumentations":
            cv2.rectangle(image, (int(bb[0] * width + 0.5), int(bb[1] * height + 0.5)), (int(bb[2] * width + 0.5), int(bb[3] * height + 0.5)), (255, 0, 255), thickness=5)
            cv2.putText(image, str(c), (int(bb[0]), int(bb[1])), cv2.FONT_HERSHEY_PLAIN, 1, (255, 0, 255))
