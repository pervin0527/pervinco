...
def load_image(b3, b5, b5_2, b7, test_set_path, test_df, class_list, mapping):
    result = []
    idx = 0
    for file_name in test_df['id']:
        ...
        b3_pred = prediction(b3, b3_image, class_list, mapping)
        b5_pred = prediction(b5, b5_image, class_list, mapping)
        b5_2_pred = prediction(b5_2, b5_image, class_list, mapping)
        b7_pred = prediction(b7, b7_image, class_list, mapping)

        # soft voting
        avg = (b3_pred[0] + b5_pred[0] + b5_2_pred[0] + b7_pred[0]) / 4
        ...
        # weight voting
        if conf < 0.9:
            weight_avg = (b3_pred[0] + b5_pred[0] + b5_2_pred[0] + (2 * b7_pred[0])) / 5
    ...
    result_df.to_csv("/data/backup/pervinco_2020/datasets/data/public/weight_soft_vote.csv", index=False)
...
load_image(b3, b5, b5_2, b7, test_set_path, test_df, CLASS_LIST, mapping)
