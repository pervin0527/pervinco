...
"""
utility functions 선언
bounding box는 여러 방법으로 나타날 수 있음 가장 흔한 format은 다음 2가지
- 모서리 좌표. VOC PASCAL format `[xmin, ymin, xmax, ymax]`
- 중심 좌표와 바운딩 박스 크기 `[x, y, width, height]`
"""

def swap_xy(boxes):
    """
    bounding box의 x, y를 swap하기 위함

    Arguments:
      bounding box를 나타내는 (num_boxes, 4) 모양의 tensor
...
def convert_to_xywh(boxes):
    """
    bounding box 형식을 center, width, height로 변경함

    Arguments:
    [xmin, ymin, xmax, ymax] format인 bounding box가 (..., num_boxes, 4)인 tensor
